...
void Graph3D::insert_edge(int x, int y, EdgeAttribute a) {
  ...
  assert(ins_x ==
         ins_y); // either none or both edges must have existed before insertion
#endif
...
int Graph3D::independent_components(vector<int> *one_of_each_component) {
  ...
  for (;;) {
    while (first_unmarked != nodes.end() && first_unmarked->second.mark != 0)
      first_unmarked++;
...
Graph3D *Graph3D::coarsen(void) {
  ...
  for (i = nodes.begin(); i != nodes.end(); i++) {
    ...
    if (curr_min_weight_node == NULL)
      curr_min_weight_node =
          &node_1; // no partner found: match node with itself
  ...
  /*  if(matching.size() < 6) {
    for(map<int,int>::iterator mi = matching.begin(); mi != matching.end();
    mi++) cout << mi->first << "->" << mi->second << " "; cout << endl;
    }*/
  ...
  Graph3D *result = new Graph3D(scene_params, use_space_grid);
  // a) set up nodes
  ...
  for (i = nodes.begin(); i != nodes.end(); i++) {
    ...
    for (j = node.neighbors().begin(); j != node.neighbors().end(); j++) {
      ...
      new_node_2_id =
          (k == matching.end() ? j->first->id() : matching[j->first->id()]);
  ...
  while (!converged) {
    ...
    for (i = nodes.begin(); i != nodes.end(); i++) {
      ...
      for (n = neighbors.begin(); n != neighbors.end(); n++) {
        ...
        float dn = delta.norm();
        if (dn <= 0)
          continue; // theta would become NaN
        f_a = dn * dn / k;
...
pair<Vector3D, Vector3D> Graph3D::compute_extremal_points(void) {
  Vector3D curr_min(FLT_MAX, FLT_MAX, FLT_MAX),
      curr_max(-FLT_MAX, -FLT_MAX, -FLT_MAX);
  map<int, Node3D>::iterator i;
  ...
  if (draw_edges) {
    ...
    for (i = nodes.begin(); i != nodes.end(); i++) {
      ...
      for (j = neighbors.begin(); j != neighbors.end(); j++) {
        ...
        if (u.id() < vp->id()) { // draw edges only in direction of incr. ids;
                                 // no self-edg.
          ...
          angle =
              180.0 / M_PI *
              acosf(Vector3D::dot_product(unit_z, dir_vec) / dir_vec.norm());
          ...
          // parameters for gluCylinder: (quad, baseRadius, topRadius, height,
          // slices, stacks);
          EdgeAttribute a = j->second;
          if (a == NT_3_PLUS_CLAUSE) {
            if (!draw_only_2clauses) {
              glMaterialfv(GL_FRONT, GL_AMBIENT_AND_DIFFUSE,
                           scene_params.color_edge_hyper);
              gluCylinder(quad, scene_params.link_width_factor * k,
                          scene_params.link_width_factor * k, dir_vec.norm(), 8,
                          4);
            }
            // else: do nothing
          } else { // 2-clause
            glMaterialfv(GL_FRONT, GL_AMBIENT_AND_DIFFUSE,
                         scene_params.color_edge_binary);
            gluCylinder(quad, scene_params.link_width_factor * k,
                        scene_params.link_width_factor * k, dir_vec.norm(), 8,
                        4);
          }
